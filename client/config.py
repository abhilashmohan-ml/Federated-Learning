...
SITE IDENTITY
--------------
Each manufacturing site runs an identical copy of the client code with its
own environment variables. Two required identity variables:
  SITE_ID     — the unique name for this site (e.g. "basel", "singapore")
  SITE_SECRET — the shared secret this site uses to authenticate with the server

Optional but commonly set:
  FLET_CLIENT_PORT — Flet UI port (default 8551)

When running via Docker Compose, each site container gets SITE_ID and
SITE_SECRET from its own environment block. When deploying to real servers,
set these as actual environment variables on the remote machine.
...
class ClientSettings(BaseSettings):
    ...
    Example .env for a site named "singapore":
        SITE_ID=singapore
    ...
    # ── Site identity ───────────────────────────────────────────────────────────
    site_id:     str = "site_1"           # any unique name for this site
    server_url:  str = "http://localhost:8000"  # URL of the FL aggregation server
    site_secret: str = ""                 # SITE_SECRET env var — shared secret for auth
